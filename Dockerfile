...
WORKDIR /rails-app
RUN /usr/bin/env bundle install --without assets test development
RUN /usr/bin/apt-get -qy purge libsqlite3-dev zlib1g-dev libghc-zlib-dev libpq-dev ruby-dev g++ make patch \
   && /usr/bin/apt-get -qy autoremove
RUN /bin/rm -rf /var/lib/gems/2.1.0/cache /var/cache/* /var/lib/apt/lists/*
ADD . /rails-app
RUN find public -mindepth 1 -not -name 'assets' \
    -not -name '.sprockets-manifest-*.json' \
    -not -name 'components-*.js' -print -delete
RUN find . -type f -print -exec chmod 444 {} \; \
