class CertificatesController < ApplicationController
  ...
  def csr
    id = params[:id]
    @csr = CertificateSignRequest.find_by certificate_id: id
    @csr ||= CertificateSignRequest.from_cert(Certificate.find(id))
    render 'csr/show'
  end
  ...
  def certificate_params
    params.require(:certificate)
          .permit(csr_attributes: [subject_alternate_names: [], subject_attributes: %i[O OU S C CN L ST]],
                  private_key_attributes: %i[bit_length type curve_name])
  end
end
